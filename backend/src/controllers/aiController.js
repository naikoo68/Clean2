...
EXPLANATION — "explanation": thorough and self-contained; put each point on its OWN line; add local/alternative names in brackets. "optionExplanations": a real JSON array of EXACTLY 4 SEPARATE notes, one per option in order (0=A,1=B,2=C,3=D), saying why each option is right/wrong; do NOT prefix entries with labels like "A)"/"Option A" and do NOT pack multiple options into one entry; leave the correct option's entry "". For CALCULATION-based questions ALSO include "numerical":true and leave ALL FOUR "optionExplanations" empty "" (the working in "explanation" is enough — no per-option notes). MATH: wrap any math/number in $...$ (never \\( \\) or \\[ \\]); NEVER use "$" for money. No markdown, no trailing commas. Return ONLY the JSON object.`;
...
function buildExtendPrompt(q, notes, fixOptions = false, extendQuestion = false) {
  ...
  if (extendQuestion) {
    lines.push(`ALSO EXTEND THE QUESTION LENGTH (this OVERRIDES the "do not change the wording" rule for the STEM ONLY): rewrite the question stem into a slightly LONGER, clearer, more descriptive version and return it as "text". ONLY EXTEND QUESTIONS THAT GENUINELY NEED IT — this is critical. Many stems are ALREADY clear, complete and self-contained and must be LEFT EXACTLY AS-IS (return the original "text" unchanged, do NOT reword or pad them): e.g. "What is the full form of NABARD?", "What is the SI unit of force?", "Who wrote …?", "In which year …?", "The capital of … is?", a simple definition, or any one-fact recall that already reads as a proper question. Extend ONLY a stem that is a bare fragment, label or too terse to read as a real question (e.g. "Lateral means:", "Photosynthesis?", "Newton's second law:") by turning it into a proper full sentence. When in doubt, DO NOT change it. STRICT LENGTH LIMIT (when you do extend): the rewritten stem must be SHORT — AT MOST 3 lines, i.e. no more than 2 sentences / about 40 words. Do NOT expand it into a paragraph; brevity matters more than extra detail — stop as soon as it is a clear, full-sentence question, and never exceed 3 lines. You MUST keep the EXACT SAME meaning, the same thing being asked, the same options and the same correct answer — only make the phrasing fuller (add a little helpful context, expand abbreviations, turn a bare label like "Lateral means:" into a proper full sentence such as "In anatomical terminology, the directional term 'lateral' refers to which of the following?"). Do NOT make the question harder, do NOT change the topic, do NOT add or reveal the answer, and do NOT turn it into a different question. Keep any real math/values wrapped in $...$ but never wrap ordinary words in $...$. For matching/assertion/statement/table questions, extend ONLY the intro sentence in "text" (still within the 3-line limit) and leave the columns/assertion/reason/table untouched.`);
  }
  return lines.join("\n");
}
