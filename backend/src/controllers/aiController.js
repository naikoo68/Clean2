...
const TYPES = ["mcq", "matching", "statement", "pair", "pairselect", "assertion", "table", "journal"];
...
// (journal entries have a distinct, meaningful correct answer like an mcq).
const ANSWER_DEDUP_TYPES = new Set(["mcq", "table", "journal"]);
...
const SHUFFLE_TYPES = new Set(["mcq", "table", "journal"]);
...
- "options": EXACTLY 4, fitting the question TYPE, with ONE genuinely correct answer and three plausible-but-wrong distractors. SAME-CATEGORY RULE (important): all four options MUST be of the SAME real-world category/type and format as the correct answer — e.g. a question about a TREE → ALL options are tree names; a river → all rivers; a person → all people of that field/era; a date → plausible nearby dates. Never mix in an unrelated kind (a flower/bird/word among tree names), and match their language, form, length and specificity so the wrong ones are closely related and believable. Wrap any numeric option value or expression in $...$ so it renders as math (e.g. "$12.5$", "$\\frac{3}{4}$", "$2^{10}$", "$25\\%$"):
  ...
  • journal: four candidate ACCOUNTING answers of the same form — each a COMPLETE journal entry / ledger posting written as a SMALL PIPE-DELIMITED TABLE with columns "Account | Debit | Credit" (a header row, then one row per account line separated by "\\n"; the debited account's amount goes in the Debit column with an empty Credit column, the credited account's amount goes in the Credit column with an empty Debit column; NO "---" separator row). Example: "Account | Debit | Credit\\nPurchases A/c | 18,000 | \\nDiscount Received A/c |  | 2,000\\nCash A/c |  | 16,000". Exactly one is correct and every entry must BALANCE (debit total = credit total). Apply the rules of debit and credit correctly; write amounts as plain numbers (NO "$", no currency symbol); the three wrong options are classic mistakes (reversed Dr/Cr, wrong account, wrong classification, wrong amount).
