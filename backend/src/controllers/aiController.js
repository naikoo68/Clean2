...
async function ytCaptionTracksViaInnerTube(id) {
  ...
  const KEY = "YOUR_YOUTUBE_INNERTUBE_KEY";
  const clients = [
...
- "optionExplanations": a real JSON array of EXACTLY 4 SEPARATE strings, in the same order as the options (entry 0 = option A, 1 = B, 2 = C, 3 = D). For a plain "mcq", leave ALL FOUR entries as empty strings "" — do NOT write per-option notes for plain MCQs (the incorrect options must not be discussed; the correct answer is covered fully in "explanation"). For every OTHER type (matching, statement, pair, pairselect, assertion, table), for EACH option state clearly whether it is correct or incorrect and WHY (for a wrong numeric option, show what mistake produces that value); keep each to 1-2 short sentences; do NOT prefix an entry with a label such as "A)", "(A)", "A." or "Option A", and do NOT put more than one option's note inside a single entry; leave the truly-CORRECT option's entry an empty string "".
...
EXPLANATION — "explanation": thorough and self-contained; since this prompt is only for plain MCQs, teach ONLY the correct option and do NOT mention or justify the incorrect options; put each point on its OWN line; add local/alternative names in brackets. "optionExplanations": leave ALL FOUR entries as empty strings "" — do NOT write per-option "why it is wrong" notes for plain MCQs. (For CALCULATION-based questions ALSO include "numerical":true — the working in "explanation" is enough.) MATH: wrap any math/number in $...$ (never \\( \\) or \\[ \\]); NEVER use "$" for money. No markdown, no trailing commas. Return ONLY the JSON object.`;
...
- EXPLANATION SCOPE BY TYPE: for a plain "mcq", the "explanation" must teach ONLY the correct option (do NOT mention or justify the incorrect options) and ALL FOUR "optionExplanations" MUST be empty strings "". For every OTHER type (matching, statement, pair, pairselect, assertion, table), the "explanation" must go through each pairing / statement / sub-option in detail, AND each of the 4 "optionExplanations" must explain why that option is right or wrong (leaving the correct option's entry "").
