...
const VizRenderer = forwardRef(function VizRenderer({ spec }, ref) {
  ...
  const engine = getModule(spec?.type)?.engine || (spec?.code ? "mermaid" : spec?.plotly ? "plotly" : spec?.graph ? "cytoscape" : spec?.framework ? "framework" : spec?.map ? "leaflet" : spec?.science ? "science" : spec?.illustration ? "illustration" : "chartjs");
